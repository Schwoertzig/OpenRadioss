...
!Copyright>        commercial version may interest you: https://www.altair.com/radioss/.
      module gradient_limitation2_superbee_mod
        implicit none
      contains
! ======================================================================================================================
!                                                   procedures
...
!! \details
      subroutine gradient_limitation2_superbee(trimat, ale_connect, numelq, nft, nel)
! ----------------------------------------------------------------------------------------------------------------------
!                                                   Modules
! ----------------------------------------------------------------------------------------------------------------------
      use ale_mod , only : ale
      use precision_mod , only : wp
      use constant_mod , only : zero, em12, ep20, one, two
...
! ----------------------------------------------------------------------------------------------------------------------
      ...
      real(kind=WP) :: grad2,grad3,norm2,delta_up,delta_down,dalpha,dy,dz,r,phi_superbee,epsilon,proj
...
! ----------------------------------------------------------------------------------------------------------------------
            ...
            if(abs(alemuscl_buffer%grad(ii,2,itrimat)) + abs(alemuscl_buffer%grad(ii,3,itrimat)) > zero) then
                   ...
                   if (norm2 > epsilon) then
                    ...
                    ! using dotproduct (gradient direction)
                     delta_up   = zero
                     delta_down = zero
                     ! loop over adjecent elems (up/down)
                     do kk = 1, 4
                       ...
                       if (iadj > 0 .and. iadj <= numelq) then
                         ...
                         ! projection along gradient direction
                     ...
                     ! --- super-bee ---
                     phi_superbee = max(zero,min(beta*r, one),min(r, beta))
                     reduc_factor(itrimat) = phi_superbee
                   else
      ...
      enddo  ! i = 1,nel
! ----------------------------------------------------------------------------------------------------------------------
      end subroutine gradient_limitation2_superbee
      end module gradient_limitation2_superbee_mod
