...
!$OMP END SINGLE
     ...
     ! VOLUME FRACTION FOR EACH FACE & NUMBERING (RETAINING MIXED CELLS ONLY)
     ! Also build reverse index: mixed_cell_id(IE) = position in compact mixed cell list (0 if pure)
     ! NOTE: tolerance must be wide enough so that near-pure cells (alpha ~ 0 or ~ 1)
     !       are NOT treated as mixed. For such cells the gradient is noise,
     ...
     ALE%VOF%cell_data%mixed_cell_id(1:NUMELQ+NUMELS) = 0
      DO IE=1,NUMELQ+NUMELS
         ...
         IF(ALPHAII > tol1 .AND. ALPHAII < tol2)then
           ...
           END DO
           !!ALE%VOF%cell_data%SWET(:, ie) = ALE%VOF%cell_data%ALPHA_F(:, ie)
           ALE%VOF%cell_data%SWET(:, IE) = ZERO
         endif
     ENDDO
     ALE%VOF%cell_data%num_mixed = num_mixed_cells


     ! NORMALIZED GRADIENT OF VOLUME FRACTION FOR EACH CELL (INTERFACE NORMAL)
     ! loop only over mixed cells to save CPU time, gradient is only needed for interface cells
     ITRIMAT = 1
     ...
     DO ICELL=1,num_mixed_cells
        ...
        ALPHA     = ALE%VOF%cell_data%ALPHA(IE)
     ...
     ! WET LENGTH ON EACH EDGE (Swet)
     ! For each edge JJ of a mixed cell, Swet(JJ) = length of the portion of edge JJ
     ! that lies on the fluid-1 side (n.x >= d).
     ! This is needed later by the advection step to compute phase fluxes through each face.
     !
     !   Case 1: both nodes inside  (D_N1 >= d, D_N2 >= d) → Swet = full edge length
     !   Case 2: both nodes outside (D_N1 <  d, D_N2 <  d) → Swet = 0
     !   Case 3: one node inside, one outside → Swet = fraction of edge length
     !
     DO ICELL=1,num_mixed_cells
        IE    = ALE%VOF%cell_data%elem_id(ICELL)
        D_VAL = ALE%VOF%cell_data%d(ICELL)
        NN(2:3) = ALE%VOF%cell_data%n(2:3, ICELL)
        NODEID(1:4) = IXQ(2:5, IE)
        ALPHA = ALE%VOF%cell_data%ALPHA(IE)
        IF(ABS(NN(2)) + ABS(NN(3)) < 1.0e-10_WP) THEN
           ! Degenerate normal: no clear interface direction.
           ! Fall back to uniform wet fraction = alpha * edge_length
           DO JJ = 1, 4
              NODEID1 = NODEID(FACE_TO_NODE_LOCAL_ID(JJ, 1))
              NODEID2 = NODEID(FACE_TO_NODE_LOCAL_ID(JJ, 2))
              Y1 = X(2, NODEID1) ; Z1 = X(3, NODEID1)
              Y2 = X(2, NODEID2) ; Z2 = X(3, NODEID2)
              EDGE_LEN = SQRT((Y2 - Y1)**2 + (Z2 - Z1)**2)
              ALE%VOF%cell_data%Swet(JJ, IE) = ALPHA * EDGE_LEN
           ENDDO
           CYCLE
        ENDIF
        DO JJ = 1, 4
           ...
           Y2 = X(2, NODEID2) ; Z2 = X(3, NODEID2)
           EDGE_LEN = SQRT((Y2 - Y1)**2 + (Z2 - Z1)**2)
           D_N1 = NN(2) * Y1 + NN(3) * Z1   ! projection of node 1
           D_N2 = NN(2) * Y2 + NN(3) * Z2   ! projection of node 2
           IF(D_N1 >= D_VAL .AND. D_N2 >= D_VAL) THEN
              ! both nodes inside → full edge is wet
              ALE%VOF%cell_data%Swet(JJ, IE) = EDGE_LEN
           ELSEIF(D_N1 < D_VAL .AND. D_N2 < D_VAL) THEN
              ! both nodes outside → edge is dry
              ALE%VOF%cell_data%Swet(JJ, IE) = ZERO
           ELSE
              ! edge crosses the interface line n.x = d
              ! intersection parameter: t such that D_N1 + t*(D_N2 - D_N1) = D_VAL
              T_CLIP = (D_VAL - D_N1) / (D_N2 - D_N1)
              T_CLIP = MAX(ZERO, MIN(ONE, T_CLIP))
              ! wet length = portion of edge on the inside
              IF(D_N1 >= D_VAL) THEN
                 ! N1 inside, N2 outside → wet portion is [0, T_CLIP]
                 ALE%VOF%cell_data%Swet(JJ, IE) = T_CLIP * EDGE_LEN
              ELSE
                 ! N1 outside, N2 inside → wet portion is [T_CLIP, 1]
                 ALE%VOF%cell_data%Swet(JJ, IE) = (ONE - T_CLIP) * EDGE_LEN
              ENDIF
           ENDIF
        ENDDO
     ENDDO

     ! NOTE: The "CLIPPING OF SWEPT POLYGON FOR ADVECTION STEP" does not belong here.
     ! It will be performed in the ALE convection routine (e.g., ale51_upwind)
     ! where the swept volume (face velocity * dt * face area) is clipped by the
     ! PLIC interface (n, d) to determine the volume of each phase being transported.
     ! The CLIPPED_AREA_QUAD4 subroutine can be reused there.
...
! ======================================================================================================================
      SUBROUTINE BRENT_FIND_D(PTS, NN, DMIN, DMAX, A_TARGET, D_OUT)
        ...
        ! If we exhausted iterations, return best guess
        D_OUT = B
